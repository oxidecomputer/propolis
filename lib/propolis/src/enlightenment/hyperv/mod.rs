...
impl HyperV {
    ...
    fn handle_wrmsr_reference_tsc(&self, value: u64) -> WrmsrOutcome {
        ...
        let mut inner = self.inner.lock().unwrap();
        if !new.enabled() {
            inner.reference_tsc_overlay.take();
        } else if let Some(mut overlay) = inner.reference_tsc_overlay.take() {
            if overlay.move_to(new.gpfn()).is_ok() {
                inner.reference_tsc_overlay = Some(overlay);
            }
        } else {
            let time_data = vmm::time::export_time_data(self.vmm_hdl())
            ...
            inner.reference_tsc_overlay = inner
                .overlay_manager
                .add_overlay(
                    new.gpfn(),
