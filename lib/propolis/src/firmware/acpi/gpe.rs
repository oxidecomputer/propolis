...
use super::{GPE0_BLK_ADDR, PCI_HOTPLUG_STATUS_ADDR};
...
impl ACPI {
    ...
    pub fn pio_rw(&self, port: u16, rwo: RWOp) {
        match port {
            ...
            PCI_HOTPLUG_STATUS_ADDR => self.pci_hotplug_rw(rwo),
            _ => panic!(),
        }
    }
    ...
    fn gpe_rw(&self, rwo: RWOp) {
        match rwo {
            RWOp::Read(ro) => {
                ...
                probes::gpe_read!(|| (ro.offset(), reg.bits()));
            }
            RWOp::Write(wo) => {
                ...
                probes::gpe_write!(|| (wo.offset(), before, reg.bits()));
            }
        }
    }
    ...
    fn pci_hotplug_rw(&self, rwo: RWOp) {
        match rwo {
            ...
            RWOp::Write(wo) => {
                ...
                probes::pci_status!(|| (wo.offset(), pci.bits()));
            }
        }
    }
...
mod probes {
    fn gpe_read(offset: usize, value: u8) {}
    fn gpe_write(offset: usize, before: u8, after: u8) {}
    fn pci_status(offset: usize, bits: u32) {}
}
