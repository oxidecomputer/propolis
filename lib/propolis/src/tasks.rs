...
impl ThreadGroup {
    ...
    ///
    /// The first (if any) [std::io::Error] encounted while among the `threads`
    /// items, will determine the return from this function.  Despite any
    /// encountered error, the non-error [thread::JoinHandle]s will be added to
    /// the group.
