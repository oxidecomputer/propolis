...
/// stack without any actual storage operations. This version uses synchronous
/// worker threads instead of async tasks.
        ...
        let spawn_results = (0..self.worker_count.get())
            .map(|n| {
                ...
                std::thread::Builder::new()
                    .name(format!("noop-sync worker {n}"))
                    .spawn(move || {
                        let wctx = wctx
                            .activate_sync()
...
impl block::Backend for NoopSyncBackend {
