...
pub struct DeviceSlotTable {
    ...
    // +1: 0 isn't a valid port ID and stays None
    port_devs: [Option<NullUsbDevice>; MAX_PORTS as usize + 1],
    log: slog::Logger,
}
...
impl DeviceSlotTable {
    pub fn new(log: slog::Logger) -> Self {
        Self {
            ...
            slots: vec![Some(DeviceSlot::new())],
            // seemingly asinine, but NullUsbDevice: !Copy,
            // which invalidates Option<>
            port_devs: [None, None, None, None, None, None, None, None, None],
            log,
        }
    }
}
...
impl DeviceSlotTable {
    ...
    pub fn detach_all_for_reset(
        ...
        self.port_devs.iter_mut().enumerate().flat_map(|(i, opt_dev)| {
            opt_dev.take().map(|dev| (PortId::try_from(i as u8).unwrap(), dev))
        })
    }
    ...
    pub fn usbdev_for_slot(
        &mut self,
        slot_id: u8,
    ) -> Option<&mut NullUsbDevice> {
        self.slot_mut(slot_id).and_then(|slot| slot.port_address).and_then(
            |idx| {
                if idx.as_raw_id() <= MAX_PORTS {
                    self.port_devs[idx.as_index()].as_mut()
                } else {
                    None
                }
            },
    ...
    fn slot_mut(&mut self, slot_id: u8) -> Option<&mut DeviceSlot> {
        self.slots.get_mut(slot_id as usize).and_then(|opt| opt.as_mut())
    }
    ...
    pub fn enable_slot(&mut self, slot_type: u8) -> Option<u8> {
        ...
        if let Some(slot_id) = slot_id_opt {
            self.slots[slot_id] = Some(DeviceSlot::new());
        }
        ...
        self.dcbaap.as_ref().and_then(|base_ptr| {
            memctx
                .read::<u64>(
                    ...
                    (*base_ptr & !0b11_1111).offset::<u64>(slot_id as usize),
                )
                .map(|x| GuestAddr(*x))
        })
    }
    ...
    pub fn address_device(
        ...
        let usb_addr = if block_set_address_request {
            ...
            if let Some(port_id) =
                self.slot(slot_id).and_then(|slot| slot.port_address)
            {
    ...
    pub fn stop_endpoint(
        ...
        Some(if let Some(Some(_)) = self.slots.get(slot_id as usize) {
            // retrieve dev ctx
            let slot_addr = self.dev_context_addr(slot_id, memctx)?;
                    ...
                    match ep_ctx.endpoint_state() {
                        EndpointState::Running => {
                            ...
                            if let Some(evt_ring) =
                                self.slot_mut(slot_id).and_then(|slot| {
                ...
                | SlotState::Configured => {
                    ...
                    match ep_ctx.endpoint_state() {
                        EndpointState::Stopped | EndpointState::Error => {
                            ...
                            // unwrap: if let Some(Some(_)) guard above
                            let slot = self
                                .slots
                                ...
                                slot.endpoints.get_mut(&endpoint_id)
                            {
                                ...
                                {
                                    slog::error!(self.log, "Error setting Transfer Ring's dequeue pointer and cycle bit for slot {slot_id}, endpoint {endpoint_id}: {e}");
                                }
                            } else {
                                slog::error!(self.log, "can't set Transfer Ring's dequeue pointer and cycle bit for slot {slot_id}'s nonexistent endpoint {endpoint_id}");
                            }
    ...
    pub fn transfer_ring(
        ...
        let Some(slot) = self.slot_mut(slot_id) else {
            slog::error!(
                log,
                "rang Doorbell for slot {slot_id}, which was absent"
            );
            return None;
        };
        let Some(endpoint) = slot.endpoints.get_mut(&endpoint_id) else {
            slog::error!(log, "rang Doorbell for slot {slot_id}'s endpoint {endpoint_id}, which was absent");
            return None;
        };
