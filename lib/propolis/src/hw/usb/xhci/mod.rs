...
For 1-based indices defined by the spec (slot ID, port ID), we put placeholder
values at position 0 of any arrays in which the ID is used as an index, such
that we aspire to categorically avoid off-by-one errors of omission (of `- 1`).
...
[Instant]s and [Duration]s rather than by repeated incrementation.
