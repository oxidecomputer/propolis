    ...
    fn imod_wait_loop(
        ...
        while let Some(pair) = pair.upgrade() {
            ...
            if !timeout_result.timed_out() {
                continue; // IMODC written, restart loop with new value of imod_allow_at
            }
            ...
            {
                // XXX: can't jump straight to this when starting thread in destination,
                // if migration attempt occurs while we've let go of the lock here
                guard.no_migrate_yet = true;
                let (mut guard, pin_timeout_result) = cvar
                    .wait_timeout_while(guard, Duration::from_secs(5), |ir| {
                        ir.pci_intr.pci_intr_mode != pci::IntrMode::INTxPin
                            || !ir.management.pending()
                    })
                    .unwrap();
                if pin_timeout_result.timed_out() {
                    slog::error!(
                        guard.log,
...
impl XhciInterrupter {
    ...
    ) -> Result<migrate::XhciInterrupterV1, crate::migrate::MigrateStateError>
    {
        ...
        let guard = if guard.no_migrate_yet {
            // TODO: more elegant/idiomatic solution
            cvar.wait_while(guard, |ir| ir.no_migrate_yet).unwrap()
        } else {
            guard
        };
    ...
    pub fn import(
        ...
        if guard.no_migrate_yet {
            guard = cvar.wait_while(guard, |ir| ir.no_migrate_yet).unwrap();
        }
