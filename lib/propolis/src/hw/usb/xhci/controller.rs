...
//! Emulated xHCI Host Controller (xHC) for support of virtual USB devices.
...
/// Contains all the relevant state for an xHC, including operational/runtime
/// registers, the Interrupter, Transfer Rings, and USB devices.
pub struct XhciState {
    ...
    /// Used for corrected timestamps, e.g. for generating MFINDEX Wrap events
    ...
    /// The consumer TRB ring on which Command TRBs are received, instantiated
    /// when a pointer to the guest-allocated buffer is written to the CRCR.
    pub(super) command_ring: Option<CommandRing>,
    /// Command Ring Control Register (CRCR).
    pub(super) crcr: bits::CommandRingControl,
    /// Manages device slots, endpoints, and other Command TRB behaviors,
    /// often involving manipulation of Device Contexts in guest memory.
    pub(super) dev_slots: DeviceSlotTable,

    /// PORTSC and friends.
    port_regs: [Box<dyn port::XhciUsbPort>; MAX_PORTS as usize],
    /// USB devices yet to be attached to a port.
    queued_device_connections: Vec<(PortId, Box<dyn UsbDevice>)>,
    /// Allocates and tracks [XhciPortHandle]s given to [UsbDevice]s, used
    /// to access port status and the Event Ring when device activity occurs.
    port_handles: XhciPortHandles,
}
...
impl XhciState {
    fn new(
        ...
        let state = Arc::new(Mutex::new(Self::new_inner(
            pci_state,
            guest_time,
            event_sender,
            log,
        )));
        state.lock().unwrap().port_handles.state_weak = Arc::downgrade(&state);
        state
    }
    ...
    fn new_inner(
        ...
        // XXX: weak ref must be set out-of-band once we are in an Arc<Mutex<>>
        let port_handles =
            XhciPortHandles::new(event_sender.clone(), Weak::new());
...
/// Plumbing for the USB device attached to a given xHC port to be able to
/// signal to the xHC that it wishes to come out of suspend, or send Event TRBs
/// upon the completion of a transfer.
pub struct XhciPortHandle {
    // used to access port_regs, which is behind the mutex holding XhciState
    state_weak: Weak<Mutex<XhciState>>,
    event_sender: Arc<EventSender>,
    port_id: PortId,
}
...
impl PciXhci {
    ...
    pub fn create(hdl: Arc<VmmHdl>, log: slog::Logger) -> Arc<Self> {
        ...
        let guest_time = VmGuestTime::new(hdl);
        let state =
            XhciState::new(&pci_state, guest_time, event_sender, log.clone());
    ...
    pub fn add_usb_device(
        ...
        let dev = device_type.create(
            hid_report,
            state.port_handles.handle_for_port(port_id),
            &self.pci_state,
            &state.guest_time,
            &self.log,
        );
        ...
        let value = match id {
            ...
            Runtime(MicroframeIndex) => {
                ...
                let mf_adjusted = state
                    .mfindex
                    .microframe_ongoing(&state.run_start, &state.guest_time);
    ...
    fn reg_write(&self, id: Registers, wo: &mut WriteOp) {
        ...
        let written_value = match id {
            ...
            Op(Configure) => {
                let mut state = self.state.lock().unwrap();
                let config = bits::Configure(wo.read_u32());
                state.config = config;
                state
                    .dev_slots
                    .set_max_slots(config.max_device_slots_enabled());
                U32(state.config.0)
            }
            Op(Port(port_id, regs)) => {
                reg_index = port_id.as_raw_id() as i16;
                ...
                if let port::PortWrite::BusReset =
                    port.reg_write(value, regs, &self.log)
                {
                    ...
                    if let Err(e) = port.xhc_update_portsc(
                        &|portsc| {
                            *portsc = portsc
                                .with_port_link_state(bits::PortLinkState::U0)
    ...
    fn usbcmd_write(&self, cmd: bits::UsbCommand) {
        let mut state = self.state.lock().unwrap();
        ...
        if cmd.run_stop() && !state.usbcmd.run_stop() {
            ...
            for (port_id, usb_dev) in queued_conns {
                ...
                if let Some(_dev) = state
                    .dev_slots
                    .attach_to_root_hub_port_address(port_id, usb_dev)
                {
                    slog::error!(
                        &self.log,
                        "root hub port {port_id:?} already had a device attached"
                    );
                }
            }
            ...
            state.run_start = Some(Arc::new(state.guest_time.now().unwrap()));
            ...
            let mf_index =
                state.mfindex.microframe_ongoing(&run_start, &state.guest_time);
        ...
        if cmd.host_controller_reset() {
            ...
            core::mem::swap(&mut devices, &mut state.queued_device_connections);
            ...
            // reset entire host state by reconstructing XhciState,
            *state = XhciState::new_inner(
                &self.pci_state,
                state.guest_time.clone(),
                event_sender,
                self.log.clone(),
            );
            // then patch up some plumbing
            state.port_handles.state_weak = Arc::downgrade(&self.state);
            ...
            // no further handling as the intent is to reset
            return;
        }
        ...
        if usbcmd_inte != state.usbcmd.interrupter_enable() {
            ...
            slog::trace!(self.log, "Interrupter Enabled: {usbcmd_inte}",);
        }
        ...
        if cmd.controller_save_state() {
            ...
            if state.usbsts.host_controller_halted() {
                ...
                state.usbsts.set_save_restore_error(false);
                slog::debug!(self.log, "unimplemented USBCMD: Save State");
            }
        }
        ...
        if cmd.controller_restore_state() {
            ...
            if state.usbsts.host_controller_halted() {
                ...
                slog::debug!(self.log, "unimplemented USBCMD: Restore State");
            }
        }
        ...
        if cmd.enable_wrap_event() && !state.usbcmd.enable_wrap_event() {
            self.start_mfindex_wrap_thread(&mut state);
        } else if !cmd.enable_wrap_event() && state.usbcmd.enable_wrap_event() {
            self.stop_mfindex_wrap_thread(&mut state);
        }
    ...
    fn start_mfindex_wrap_thread(&self, state: &mut XhciState) {
        let Some(run_start_arc) = state.run_start.as_ref() else { return };
        let initial_value = state.mfindex.microframe() as u32;
        let guest_time = state.guest_time.clone();
        let first_wrap_time = run_start_arc
            ...
            .unwrap_or_else(|| {
                ...
                guest_time.now().unwrap()
            });

        let generation = state.mfindex_wrap_thread_generation.wrapping_add(1);
        state.mfindex_wrap_thread_generation = generation;

        let state_weak = Arc::downgrade(&self.state);

        let mfindex_thread_body = move || {
            ...
            loop {
                ...
                if state.mfindex_wrap_thread_generation == generation {
                    state
                        .event_sender
                        .enqueue_event(EventInfo::MfIndexWrap, false)
        ...
        let mfindex_thread_jh = std::thread::Builder::new()
            .name("xHCI mfindex wrap thread".to_string())
            .spawn(mfindex_thread_body)
            .unwrap();
        state.mfindex_wrap_thread = Some(mfindex_thread_jh);
    }
...
impl MigrateMulti for PciXhci {
    ...
    fn import(
        ...
        state.command_ring = command_ring
            ...
            .transpose()?;
        let guest_time = state.guest_time.clone();
        let config_max_slots = state.config.max_device_slots_enabled();

        // borrow chess
        {
            let XhciState { dev_slots, port_handles, .. } = &mut *state;
            dev_slots.import(
                &dev_slots_v1,
                ctx,
                port_handles,
