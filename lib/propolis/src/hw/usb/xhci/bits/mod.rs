...
impl MicroframeIndex {
    ...
    pub fn microframe_ongoing(&self, run_start: &Option<Arc<Instant>>) -> u16 {
        let elapsed_microframes = run_start
            .as_ref()
            .and_then(|then| Instant::now().checked_duration_since(**then))
            // NOTE: div_duration_f32 not stablized until 1.80, MSRV is 1.70
            .map(|duration| {
                duration.as_secs_f64() / MINIMUM_INTERVAL_TIME.as_secs_f64()
            })
            .unwrap_or_default();
        self.microframe().wrapping_add(elapsed_microframes as u16)
    }
}
