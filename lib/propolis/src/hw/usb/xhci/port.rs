...
// HACK: port ID's begin at 1, so we stuff this dummy value in ports[0]
// (an array of Box<dyn XhciUsbPort>)
#[derive(Default)]
pub struct NilPort(bits::PortStatusControl);
impl XhciUsbPortPrivate for NilPort {
    fn is_usb3(&self) -> bool {
        false
    }
    fn portsc_ref(&self) -> &bits::PortStatusControl {
...
impl XhciUsbPort for NilPort {
    fn portpmsc_write(&mut self, _value: u32, _log: &slog::Logger) {}
    fn portli_write(&mut self, _value: u32, _log: &slog::Logger) {}
    fn porthlpmc_write(&mut self, _value: u32, _log: &slog::Logger) {}
}
