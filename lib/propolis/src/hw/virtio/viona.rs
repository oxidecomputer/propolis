...
pub const PROPOLIS_MAX_MQ_PAIRS: u16 = 8;
...
impl PciVirtioViona {
    pub fn new(
        ...
        Self::new_with_queue_sizes(
            vnic_name,
            TX_QUEUE_SIZE.try_into().unwrap(),
            RX_QUEUE_SIZE.try_into().unwrap(),
            CTL_QUEUE_SIZE.try_into().unwrap(),
            vm,
            viona_params,
        )
    }
    ...
    fn ctl_queue_notify(&self, vq: &Arc<VirtQueue>) {
        if let Some(mem) = self.pci_state.acc_mem.access() {
    ...
    fn set_use_pairs(&self, requested: u16) -> Result<(), ()> {
        ...
        self.virtio_state.queues.set_len(npairs * 2 + 1);
        Ok(())
    }
    ...
    fn queues_restart(&self) -> Result<(), ()> {
        ...
        for vq in self.virtio_state.queues.iter() {
            ...
            if !vq.is_control() && self.hdl.ring_reset(vq.id).is_err() {
                *rs = VRingState::Fatal;
                res = Err(());
            ...
            *rs = VRingState::Init;
            if vq.is_mapped() {
                ...
                if let Some(intr_cfg) = vq.read_intr() {
                    if !vq.is_control()
                        && self.hdl.ring_cfg_msi(vq.id, Some(intr_cfg)).is_err()
                    {
                        *rs = VRingState::Error;
                        continue;
                    }
        ...
        match change {
            ...
            VqChange::Address => {
                match *rs {
                    ...
                    | VRingState::Error => {
                        ...
                        if !vq.is_control()
                            && self.hdl.ring_reset(vq.id).is_err()
                        {
                            *rs = VRingState::Fatal;
                            return Err(());
                        }
                ...
                if !vq.is_control() && self.hdl.ring_init(vq.as_ref()).is_err()
                {
            ...
            VqChange::IntrCfg => {
                if *rs != VRingState::Fatal {
                    ...
                    if !vq.is_control()
                        && self.hdl.ring_cfg_msi(vq.id, intr).is_err()
                    {
                        *rs = VRingState::Error;
                    }
                }
...
impl VionaHdl {
    ...
    fn intr_poll(
        ...
        let mut vna_ip = viona_api::vioc_intr_poll::default();
        vna_ip.vip_nrings = max_intrs as u16;
        let mut nintrs =
            unsafe { self.0.ioctl(viona_api::VNA_IOC_INTR_POLL, &mut vna_ip)? };
        let nrings = vna_ip.vip_nrings as usize;
        for i in 0..nrings {
            let k = i / 32;
            let b = i % 32;
            if vna_ip.vip_status[k].get_bit(b) {
                f(i as u16);
                nintrs -= 1;
                if nintrs == 0 {
                    break;
    ...
    fn set_notify_mmio_addr(&self, addr: Option<u64>) -> io::Result<()> {
        let vim_address = addr.unwrap_or(0);
        let vim_size = super::pci::NOTIFY_REG_SIZE as u32;
        let mut vim = viona_api::vioc_nofity_mmio { vim_address, vim_size };
        unsafe {
            self.0.ioctl(viona_api::VNA_IOC_SET_NOTIFY_MMIO, &mut vim)?;
        }
        Ok(())
    }
