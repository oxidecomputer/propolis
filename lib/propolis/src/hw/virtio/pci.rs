...
impl VirtioState {
    fn new(num_queues: u16, mode: virtio::Mode) -> Self {
...
impl<D: PciVirtio + Send + Sync + 'static> pci::Device for D {
    ...
    fn bar_rw(&self, bar: pci::BarN, mut rwo: RWOp) {
        ...
        let map = match bar {
            pci::BarN::BAR0 => {
                if vs.legacy_map_use_msix.load(Ordering::Acquire) {
                    &vs.legacy_config
                } else {
    ...
    fn cap_rw(&self, id: CapId<u32>, mut rwo: RWOp) {
        ...
        match id {
            VirtioCfgCapTag::Common => {
                COMMON_CFG_CAP_REGS.process(&mut rwo, |id, rwo| {
                    if let RWOp::Read(ro) = rwo {
                        vs.common_cfg_cap_read(id, ro);
                    }
                });
            }
            VirtioCfgCapTag::Notify => {
                NOTIFY_CFG_CAP_REGS.process(&mut rwo, |id, rwo| {
                    if let RWOp::Read(ro) = rwo {
                        vs.notify_cfg_cap_read(id, ro)
                    }
                });
            }
            VirtioCfgCapTag::Isr => {
            ...
            VirtioCfgCapTag::Vendor => {
                unimplemented!("VirtIO Vendor capabilities are unsupprted");
            }
        }
    }
...
impl PciVirtioState {
    pub(super) fn create(
        ...
        let device_id = device_id.to_pci_dev_id(mode).expect("Device ID");
        ...
        let common_config = RegMap::create_packed_passthru(
            ...
            &[
                ...
                (
                    VirtioConfigRegBlock::WiR0,
                    0x1000 - (COMMON_REG_SIZE + cfg_size),
                ),
                (VirtioConfigRegBlock::Notify, NOTIFY_REG_SIZE),
                (VirtioConfigRegBlock::WiR0, 0x1000 - NOTIFY_REG_SIZE),
                (VirtioConfigRegBlock::IsrStatus, ISR_STATUS_REG_SIZE),
                (VirtioConfigRegBlock::WiR0, 0x2000 - ISR_STATUS_REG_SIZE),
            ],
        );
        ...
        let state = Mutex::new(VirtioState::new(queue_count, mode));
    ...
    fn common_read(
        ...
        match id {
            CommonConfigReg::DeviceFeatureSelect => {
                ...
                ro.write_u32(state.device_feature_select.into());
            }
            ...
            CommonConfigReg::QueueEnable => {
                ...
                let enabled = self
                    .queues
                    .get(state.queue_select)
                    .map(|q| q.enabled.load(Ordering::Acquire))
                    .unwrap_or(false);
    ...
    fn common_write(
        ...
        match id {
            ...
            CommonConfigReg::DriverFeature => {
                ...
                if shift < 64 {
                    let current = {
                        ...
                        state.negotiated_features.get_bits(lo..hi)
                    };
            ...
            CommonConfigReg::QueueSize => {
                ...
                self.queues.get(state.queue_select).map(|q| {
                    let mut size = q.size.lock().unwrap();
                    *size = offered
                });
            }
            CommonConfigReg::QueueMsixVector => {
            ...
            CommonConfigReg::QueueEnable => {
                let state = self.state.lock().unwrap();
                let value = wo.read_u16() != 0;
                self.queues
                    .get(state.queue_select)
                    .map(|q| q.enabled.store(value, Ordering::Release));
            }
            CommonConfigReg::QueueNotifyOffset => {
                // Read-only for driver
            }
            CommonConfigReg::QueueDescAddr => {
                let mut state = self.state.lock().unwrap();
                let offered = wo.read_u64();
                if let Some(queue) = self.queues.get(state.queue_select) {
                    let current = queue.get_state();
                    queue.map_legacy(offered);
                    if current.mapping.desc_addr != offered {
            ...
            CommonConfigReg::QueueDriverAddr => {
                ...
                if let Some(queue) = self.queues.get(state.queue_select) {
                    ...
                    if current.avail_addr != offered_avail_addr {
                        if dev.queue_change(queue, VqChange::Address).is_err() {
                            self.needs_reset_locked(dev, &mut state);
                        }
                    }
            ...
            CommonConfigReg::QueueDeviceAddr => {
                ...
                if let Some(queue) = self.queues.get(state.queue_select) {
                    ...
                    if current.used_addr != offered_used_addr {
                        if dev.queue_change(queue, VqChange::Address).is_err() {
                            self.needs_reset_locked(dev, &mut state);
                        }
                    }
    ...
    fn common_cfg_cap_read(&self, id: &CommonCfgCapReg, op: &mut ReadOp) {
        match id {
            ...
            CommonCfgCapReg::Offset => op.write_u32(0),
    ...
    fn notify_cfg_cap_read(&self, id: &NotifyCfgCapReg, op: &mut ReadOp) {
        match id {
            NotifyCfgCapReg::Common(common_id) => match common_id {
                ...
                CommonCfgCapReg::Offset => op.write_u32(0x1000),
                CommonCfgCapReg::Length => op.write_u32(4),
    ...
    fn device_cfg_cap_read(&self, id: &CommonCfgCapReg, op: &mut ReadOp) {
        match id {
            ...
            CommonCfgCapReg::Offset => op.write_u32(COMMON_REG_SIZE as u32),
            CommonCfgCapReg::Length => op.write_u32(12),
    ...
    fn isr_cfg_cap_read(&self, id: &CommonCfgCapReg, op: &mut ReadOp) {
        match id {
            ...
            CommonCfgCapReg::Offset => op.write_u32(0x2000),
            CommonCfgCapReg::Length => op.write_u32(1),
            _ => self.common_cfg_cap_read(id, op),
        }
    }
    ...
    fn set_intr_mode(
        ...
        self.legacy_map_use_msix
            .store(new_mode == IntrMode::Msi, Ordering::Release);
        match new_mode {
            IntrMode::IsrLintr => {
...
impl MigrateMulti for PciVirtioState {
    fn export(
        ...
        let device = migrate::DeviceStateV1 {
            ...
            queue_sel: state.queue_select,
            nego_feat: state.negotiated_features,
    ...
    fn import(
        ...
        state.queue_select = dev.queue_sel;
        state.negotiated_features = dev.nego_feat;
