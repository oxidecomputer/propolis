...
//! API request and response types for the XHCI_USB API version.
...
impl From<v3::api::InstanceInitializationMethod>
    for InstanceInitializationMethod
{
    fn from(old: v3::api::InstanceInitializationMethod) -> Self {
        match old {
            v3::api::InstanceInitializationMethod::Spec { spec } => {
                Self::Spec { spec: spec.into() }
            }
            v3::api::InstanceInitializationMethod::MigrationTarget {
                migration_id,
                src_addr,
                replace_components,
...
impl From<v3::api::InstanceEnsureRequest> for InstanceEnsureRequest {
    fn from(old: v3::api::InstanceEnsureRequest) -> Self {
