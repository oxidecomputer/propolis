...
#[phd_testcase]
async fn native_acpi_tables_in_spec(ctx: &TestCtx) {
...
#[phd_testcase]
async fn native_tables_preserved_on_migration(ctx: &TestCtx) {
    let mut source =
...
#[phd_testcase]
async fn ovmf_tables_preserved_on_migration(ctx: &TestCtx) {
...
mod from_base {
    ...
    #[phd_testcase]
    async fn ovmf_tables_preserved_through_migrations(ctx: &TestCtx) {
