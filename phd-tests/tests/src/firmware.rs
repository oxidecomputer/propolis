...
// This test uses ACPI tools to verify that the ACPI tables generated by
// Propolis are functional.
//
// It requires a guest OS image with the following tools installed:
//   - acpidump
//   - iasl
//   - fwts
//
// fwts doesn't build very easily on Alpine, so a Debian/Ubuntu based image is
// recommended.
//
// The ACPICA tools can be installed from packages:
//   apt install acpica-tools acpi
//
// fwts can be installed from source:
//   https://github.com/fwts/fwts/blob/master/README
#[phd_testcase]
async fn acpi_tables_parse(ctx: &TestCtx) {
    ...
    // The current ACPI tables generate (num_cpus + 2) errors and 1 warning.
    //
